...
export default function CategoryContainer() {
  ...
  const { addData, updateData, deleteData } = useCategoryStore();
      ...
      onOk: async () => {
        try {
          ...
          notify({
            type: "error",
            notifyTitle: "Error!",
            notifyContent: (error as any).message,
          });
        }
      },
    });
  ...
  return (
    <section className="bg-white w-full h-full p-5 rounded-lg overflow-x-auto">
      ...
      {/* Modal Form */}
      <DefaultModal
        modalTitle={editingData ? "Edit Kategori" : "Tambah Kategori"}
        isOpen={isModalOpen}
        handleOk={handleOk}
        handleCancel={() => {
          closeModal();
